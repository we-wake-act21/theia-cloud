...
resource "helm_release" "theia-cloud-base" {
  ...
  chart            = "../../../helm/charts/theia.cloud-base"
...
resource "helm_release" "keycloak" {
  ...
  provisioner "local-exec" {
    command     = "kubectl patch deploy ingress-nginx-controller --type=json -n ingress-nginx -p '[{ \"op\": \"add\", \"path\": \"/spec/template/spec/containers/0/args/-\", \"value\": \"--default-ssl-certificate=keycloak/${var.hostname}-tls\" }]'"
    interpreter = ["PowerShell", "-Command"]
  }
  provisioner "local-exec" {
    command = "kubectl wait pods -n ingress-nginx -l app.kubernetes.io/component=controller --for condition=Ready"
  }
  provisioner "local-exec" {
    command = "kubectl wait certificate -n keycloak ${var.hostname}-tls --for condition=Ready"
  }
}
...
resource "helm_release" "theia-cloud" {
  ...
  values = [
    "${templatefile("${path.module}/theia-cloud.yaml", { landing_hostname = var.landing_hostname, service_hostname = var.service_hostname, instance_hostname = var.instance_hostname })}",
  ]
